...
版本 3.0 新增品項級資源庫存、抽象資源換算、逐回合消耗帳本，以及只使用匿名化摘要的 LLM 次回合想定生成。系統仍包含 **STORM（Synthetic Theater Operations Research Model）概念分析實驗室**；本模組是依公開文獻重新設計的模擬版，不是正式 STORM 軟體或其演算法重製。
...
- 抽象換算：固定規則將品項換算為航空、防空、遠程火力、海上、水下、ISR、通訊、後勤及能源等 0–100 指數。
- 回合資源帳本：每回合保存期初、恢復、補充、行動消耗、事件損失與期末數量。
- 次回合想定包：依結算後資源趨勢生成情報、候選事件與決策難題；有 API Key 時可使用 LLM，否則自動採本機規則。
- 敏感資料模式：品項名稱、原始數量及備註只留在瀏覽器；LLM 最多只接收匿名化類別分數與趨勢。
