...
- 回合兵推：藍方、紅方、琥珀方每回合提交命令包（1項主行動＋2–4項支援行動），在共用資源上限下由規則引擎與事件卡產生抽象結果。
- 抽象作戰類型：新增航空攔截、防空交戰、遠程火力、海上拒止及水下警戒等教學行動；不含真實武器型號、射程、座標、目標、彈藥量或交戰程序。
...
- AI 回合補齊：輸入 API Key 後，「自動補齊其他角色」會依當前資源、準備度、情報、事件、天候與已提交命令生成缺少角色的合成命令包，並顯示理由；API 不可用時回退至本機合成規則。
...
`data/demo-data.js`、SQLite與Excel目前同步保存9種情境範本、62個介面設定、11項合成資源、38項回合行動及4個LLM供應商預設值。API Key不會寫入任何資料庫檔案。
